"""
PivotBuilder dataclass that validates and generates pivot tables from Excel tables.
"""
...
class PivotBuilder:
    """
    Builder for creating a pivot table from an Excel Table (ListObject).
    ...
    Args:
        ...
        row_fields: Row field definitions in order (outer to inner).
        column_fields: Column field definitions in order (outer to inner).
        data_fields: Data field definitions in order.
        destination_handling: How to find/create/clear the destination sheet (required).
    ...
    Limitations:
        ...
        - DestinationHandling.FIND_OR_CREATE uses an empty sheet if present, otherwise creates one.
    """
    ...
    column_fields: List[ColumnField] = field(default_factory=list)
    ...
    def generate_pivot(self) -> None:
        """
        Validate the builder and generate the pivot table.
